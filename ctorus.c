...
int main (int argc, char **argv)
{
   ...
   if (argc!=3) {
     fprintf(stderr,"CTORUS: need parameters.\n");
	 fprintf(stderr,"  usage: CTORUS <x increments> <y increments>\n");
	 exit(-1);
   }
   maxx=atoi(argv[1]);
   maxy=atoi(argv[2]);
   ...
   /* Connect lines on other side. */
   for (i=0; i<maxx; i++) {
    printf("%d %d\n",i,i+(maxx*(maxy-1)));
   }

   /* Name field. Modify. */
   printf("n=CliffordTorus-%dx%d\n",maxx,maxy);
